...
{
  // Declarations
  printf("Enter sp computer");
  KShorthestPathGraph::BaseGraphType & G_other = PathGraph.GetBaseGraph();
  KShorthestPathGraph::BaseGraphType  G;
  printf("Before Copy");
  boost::copy_graph(G_other,G);
  int nNoOfOrigNodes_other = boost::num_vertices(G_other);
  int nNoOfOrigNodes_G = boost::num_vertices(G);
  std::cout<< nNoOfOrigNodes_other <<" vs " << nNoOfOrigNodes_G<< std::endl;
  ...
  for( nPathCounter = 0; nPathCounter < nMaxNoOfPaths; nPathCounter++ )
    {
      ...
      TransformShortestPath_G( G, nSourceNode, nTerminalNode, Preds, 
...
void KShorthestPathComputer::TransformShortestPath_G( 
                                                   KShorthestPathGraph::BaseGraphType & G,
                                                   int nSourceNode, int nTerminalNode,
                                                   std::vector< int >& rPreds, 
                                                   int* pnReversedNodeMap,  
